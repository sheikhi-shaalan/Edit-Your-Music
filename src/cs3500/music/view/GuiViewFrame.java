...
public class GuiViewFrame extends JFrame implements GuiView, Playable {
  ...
  /**
   * Creates new GuiView with the given MusicCreator The default size is 800 by 600. The default
   * close operation is to exit on close Adds a scroll pane to the MusicCreator and sets its
   * preferred size to be 600 by 200
   */
  ...
  public Note userNote() throws NumberFormatException, IllegalArgumentException {
    Note ret = null;
    Note.Pitch[] pitches = {Note.Pitch.C, Note.Pitch.CSHARP,
            Note.Pitch.D, Note.Pitch.DSHARP, Note.Pitch.F,
            Note.Pitch.FSHARP, Note.Pitch.G, Note.Pitch.GSHARP,
            Note.Pitch.A, Note.Pitch.ASHARP, Note.Pitch.B};
  ...
  public void skipToEnd() {
    this.scrollPane.getHorizontalScrollBar().setValue(c.getSongDuration()
            * displayPanel.PIXEL_SIZE);
